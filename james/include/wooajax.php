...
function product_quickview() {
	global $product, $post, $woocommerce_loop, $james_opt;
	...
	<div class="woocommerce product">
		...
		<div class="product-info">
			...
			<a class="see-all" href="<?php echo esc_url($product->get_permalink()); ?>"><?php echo esc_html($james_opt['quickview_link_text']); ?></a>
			<div class="quick-add-to-cart">
				<?php woocommerce_template_single_add_to_cart(); ?>
			</div>
			<div class="quick-desc"><?php echo do_shortcode(get_post($productid)->post_excerpt); ?></div>
			<div class="social-sharing"><?php james_product_sharing(); ?></div>
		</div>
	</div>
	<?php
	die();
...
function get_cartinfo() {
	...
	echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'james'), $woocommerce->cart->cart_contents_count);
...
function get_productinfo() {
	global $product, $woocommerce_loop, $james_opt;
	
	$productid = intval( $_POST['data']['pid'] );
	$product = get_product( $productid );
	?>
	<h3><?php esc_html_e('Product is added to cart', 'james');?></h3>
	...
	<div class="buttons">
		<a class="button" href="<?php echo get_permalink( wc_get_page_id( 'cart' ) );?>"><?php esc_html_e('View Cart', 'james');?></a>
	</div>
	<?php
	die();
}
